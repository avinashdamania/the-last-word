...
class DeveloperVisualizationPage3 extends React.Component {
    constructor(props) {
        super(props);
        this.state =  {data: default_data , isLoading:false};
        var end_point = "https://dontdieyoung.me/api/workout";
        
        fetch(end_point).then(response => {
            console.log(response)
            return response.json();
        }).then(data => {
            var equipment_map = new Map()
            data.forEach(item => {
                ...
                var temp2 = item.equipment
                var temp3 = temp2.split("/")
                temp3.forEach(temp => {
                    if (equipment_map.has(temp)) {
                        equipment_map.set(temp, equipment_map.get(temp) + 1)
                    } else {
                        equipment_map.set(temp, 1)
                    }
            ...
            var actual_data = []
            for (const [key, value] of equipment_map.entries()) {
                console.log("hi")
            ...
            <span style={{display: "flex", justifyContent: "center", alignItems: "center"}}>
                {this.state.isLoading
                    ...
                    <div style={{backgroundColor: "white", margin: 20, padding: 20, borderRadius: 20}} >
                        ...
                        <h2>A visualization for the equipment needed for various workouts from our developer's data.</h2>
                        <span>
...
export default DeveloperVisualizationPage3;
